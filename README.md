...
* ### Which architecture is more secure? 2 tier or 3 tier
  ...
  * ##### Layers
    * ##### 1. Client Layer (UI)
    * ##### 2. Data Layer
  * ##### In two-tier, the application logic is either buried inside the UI, on the client or within the databse, on the server (or both). With two-tier client/server architectures, the user system interface is usually located in the user's desktop environment and the databse management services are usually in a server (because it is a more powerful machine)
  * ##### The user system interface is the Ui
  * ##### The Database manangement system (DBMS) are programs like MySQL, MS SQL Server, Oracle.
  #### 2. Three-tier Databse Architecture
  * ##### Layers
    * ##### 1. Client Layer (UI)
    * ##### 2. Middle Layer (Buisness Logic)
    * ##### 3. Data Layer
  * ##### In three-tier. the appication logic or process lives in a middle-tier. It is seperated from the server and the client.
  * ##### Pros: More scalable, robust, flexible, can integrate data from many sources, more secure
